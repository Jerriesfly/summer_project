...
<%
    ...
    if(request.getParameter("id") != null){
        isModify = true;
        try {
            travelImage = new TravelImageDao().getImageDetail(Integer.parseInt(request.getParameter("id")));
    ...
    <div class="content">
        <form onsubmit="return checkForm()" method="post"
            ...
            <div id="preview">
                <%
                    if(isModify){
                        out.println("<img src='" + request.getContextPath() + "/img/travel-images/medium/" + travelImage.getPath() + "'>");
                    } else{
                        out.println("<span>Photo not uploaded yet.</span>");
                    }
                %>
            </div>

            <div id="upload">
                <label for="file0" id="label-file"><%= isModify ? "modify" : "upload photo" %></label>
            ...
            <div id="information">
                ...
                <select name="country" id="country">
                    <option id="country_default" value="" selected>请选择国家</option>
...
<script src="${pageContext.request.contextPath}/js/filterOnchange.js"></script>
<script>
...
<script defer="defer">
    ...
    for (let i = 0; i < content.length; i++) {
        if (content.options[i].value === '<%= travelImage.getContent()%>') {
            content.options[i].selected = true;
        }
    }

    for (let i = 0; i < select_country.length; i++) {
        if (select_country.options[i].value === '<%= travelImage.getCountryName()%>') {
            select_country.options[i].selected = true;
            select_country.onchange();
        }
    }

    for (let i = 0; i < select_city.length; i++) {
        if (select_city.options[i].value === '<%= travelImage.getCityName()%>') {
...
<script>
    function checkForm() {
        if (content.selectedIndex && select_country.selectedIndex && select_city.selectedIndex) {
            return true;
        } else {
